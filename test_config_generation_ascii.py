...
class ConfigGenerationTester:
    ...
    def run_all_tests(self):
        """Run all test cases"""
        print("=" * 80)
        print("TEST: COMPREHENSIVE CONFIGURATION GENERATION TEST SUITE")
        print("=" * 80)
        print()
    ...
    def run_test(self, test_case):
        ...
        from pathlib import Path
        test_config_dir = Path('output/test_configs')
        ...
        try:
            ...
            # Generate configuration text using to_string method
            from phase5.config_generator import ConfigGenerator
            gen = ConfigGenerator()
            config_text = gen.to_string(config, format_style='cli')
            
            # Save generated configuration to file
            safe_name = test_name.lower().replace(' ', '_').replace('/', '_')
            config_file = test_config_dir / f"{safe_name}.txt"
            with open(config_file, 'w') as f:
                f.write(f"# Test: {test_name}\n")
                f.write(f"# Requirement: {requirement}\n")
                f.write(f"# Modules Generated: {len(config.modules)}\n")
                f.write(f"# Validation: {'VALID' if validation.is_valid else 'INVALID'}\n")
                f.write(f"\n{'='*80}\n\n")
                f.write(config_text)
                f.write(f"\n\n{'='*80}\n")
            ...
            if missing_modules:
                ...
                with open(config_file, 'w') as f:
                    ...
                    f.write(f"# Requirement: {requirement}\n")
                    f.write(f"# Error: Missing expected modules: {missing_modules}\n")
                    f.write(f"# Generated modules: {actual_modules}\n")
                    f.write(f"\n{'='*80}\n\n")
                    f.write(config_text)
            ...
            # Test passed
            self.record_pass(test_name, f"Generated {len(actual_modules)} module(s) -> {config_file}")
